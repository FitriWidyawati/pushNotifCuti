...
@Component({
  selector: 'page-pengajuan-cuti',
  templateUrl: 'pengajuan-cuti.html'
})
export class PengajuanCutiPage {
  ...
  goToMonitoringPengajuanCuti(params){
    ...
    this.navCtrl.push(MonitoringPengajuanCutiPage);
  	...
  	sendNotificationWithActionButtonsAndDeepLink(){
  		window["plugins"].OneSignal.getIds(function(ids) {
  			var notificationObj = { contents: {en: "Permohonan Cuti"},
                          include_player_ids: [ids.userId],
                          data: {data_key: "data_value", openURL: "https://imgur.com/"},

                          buttons: [{"id": "id1", "text": "Setujui", "icon": "checkbox_on_background",}, {"id": "id2", "text": "Tolak Pengajuan", "icon": "ic_delete"}]
                          };
